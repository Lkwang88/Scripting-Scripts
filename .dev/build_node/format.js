...
exports.relTimeShort = relTimeShort;
exports.clockTime = clockTime;
exports.fmtRtt = fmtRtt;
exports.uptimeRatio = uptimeRatio;
exports.fmtUptime = fmtUptime;
exports.avgRtt = avgRtt;
...
function relTimeShort(ts, now = Date.now()) {
    if (!ts)
        return "—";
    const sec = Math.max(0, Math.round((now - ts) / 1000));
    if (sec < 60)
 ...
 */
function sparkPoints(history, n) {
    if (!history.length || n <= 0)
        return [];
...
/** 主机在界面上的次要说明行 */
function stateHint(state, now = Date.now()) {
    switch (state.status) {
        case "offline":
